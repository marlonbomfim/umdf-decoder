...
Message::Message() : seq_num(-1), num_chunks(-1), total_size(0) {
}

Message::Message(const Packet& p) :
    seq_num(p.seqnum()), num_chunks(p.num_chunks()),total_size(0) {
  add(p);
}
...
void Message::add(const Packet& p) {
  ...
  if(p.seqnum()!=seq_num) throw 1; //TODO: error
  packets.push_back(p);
  total_size+=p.length();
...
int Message::read(void* dest,int offset,int len) const {
  ...
  int total_to_read=total_size-offset<len?total_size-offset:len;
